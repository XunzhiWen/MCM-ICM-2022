...
\begin{algorithm}[htbp]
    ...
    \begin{algorithmic}[1]
      \Require
        ...
        $ V_t = (V_{t-1}+a\rm{DBH}^bh^c)\times $$Area\times Density(t) \times (1-\lambda(t)) $ \citep{LuoQingbang}
        \EndFor

        \noindent$ Area = Area_b + Area_c $

        \noindent $ \rm{CarbonMass_f} $  $= \beta_cV_c+\beta_bV_b+9.6\times Area_c+3.4\times Area_b + 70\times Area$ \citep{YanDeren2011}
